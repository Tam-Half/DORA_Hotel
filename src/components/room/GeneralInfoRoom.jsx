...
export default function GeneralInfoRoom({ room }) {
  ...
  return (
    <div className="h-fit rounded-xl flex justify-between p-4">
      <div className="w-3/4">
        <h2 className="text-3xl font-bold p-2">{room.name || 'Delux Ocean View Suite'}</h2>

        <div className="flex w-2/3 text-xs justify-between p-2">
          <span>{room.address || 'Vung Tau, Viet Nam - Cach Bien 200m'}</span>

          <div className="flex w-1/3 gap-1 items-center">
            <img src={star} alt="" className="w-4" />
            <span className="font-bold">{room.averageRating || '4.9'}</span>
            <span>({room.totalReviews || '128'} đánh giá)</span>
          </div>
        </div>
      </div>
